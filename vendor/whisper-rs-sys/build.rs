...
fn main() {
    ...
    if cfg!(windows) {
        let _: u64 = std::fs::copy("src/bindings_windows.rs", out.join("bindings.rs"))
            .expect("Failed to copy bindings_windows.rs");
    } else if env::var("WHISPER_DONT_GENERATE_BINDINGS").is_ok() {
